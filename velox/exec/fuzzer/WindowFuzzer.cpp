...
T WindowFuzzer::genOffsetAtIdx(
  ...
  if (frameBoundType == core::WindowNode::BoundType::kPreceding) {
    if (orderByKey.sortOrder_.isAscending()) {
...
std::optional<std::string> WindowFuzzer::addKRangeFrameColumnToInput(
  ...
  if constexpr (!(std::is_same_v<T, bool> ||
    ...
    while (offsetValue < 0) {
      fuzzOffset = vectorFuzzer_.fuzzConstant(type, 1);
      offsetValue = fuzzOffset->as<ConstantVector<T>>()->valueAt(0);
    }
...
std::string WindowFuzzer::generateFrameClause(
  ...
  auto frameString = core::WindowNode::windowTypeName(windowType);
  ...
  auto frameBound = [&](core::WindowNode::BoundType boundType,
                        std::string columnName) -> const std::string {
  ...
  auto frameStart = frameBound(startBoundType, "k0");
  auto frameEnd = frameBound(endBoundType, "k1");

  return fmt::format("{} BETWEEN {} AND {}", frameString, frameStart, frameEnd);
}
