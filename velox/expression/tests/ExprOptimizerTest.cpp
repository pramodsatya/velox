...
class ExprOptimizerTest : public functions::test::FunctionBaseTest {
 protected:
  ...
  /// @param input String representation of input SQL expression.
  /// @param expected String representation of expected SQL expression.
  /// @param type Type of `input`.
  /// @param failResult Replace failed subexpression optimizations with a fail
  /// expression.
  /// @param evalError Expected error message upon evaluating `input`,
  /// `failResult` should be true for validating error.
  void testExpression(
    ...
    auto optimized =
        expression::optimize(typedExpr, queryCtx_.get(), pool(), failResult);
...
TEST_F(ExprOptimizerTest, failExpression) {
  auto failExpression = [&](const std::string& error, const TypePtr& type) {
    return fmt::format("cast(fail('{}') as {})", error, type->toString());
  };
  auto assertFailCall = [&](const std::string& expression,
                            const std::string& expected,
                            const std::string& error,
                            const RowTypePtr& type = ROW({})) {
    testExpression(expression, expected, type, true, error);
  };

  // Primitive types.
  const auto error = "division by zero";
  assertFailCall("0 / 0", failExpression(error, BIGINT()), error);
  assertFailCall(
      "if(false, a * abs(-1 * 3), 0 / 0)",
      fmt::format("if(false, a * 3, {})", failExpression(error, BIGINT())),
      error,
      ROW({"a"}, {BIGINT()}));
  assertFailCall(
      "json_extract(a, substr(b, 1 / 0))",
      fmt::format(
          "json_extract(a, substr(b, {}))", failExpression(error, BIGINT())),
      error,
  ...
  assertFailCall(
      "filter(a, x -> (1 / 0) > 1)",
      fmt::format("filter(a, x -> {} > 1)", failExpression(error, BIGINT())),
      error,
      ROW({"a"}, {ARRAY(BIGINT())}));
  assertFailCall(
      "map_top_n(a, 1 / 0)",
      fmt::format("map_top_n(a, {})", failExpression(error, BIGINT())),
      error,
