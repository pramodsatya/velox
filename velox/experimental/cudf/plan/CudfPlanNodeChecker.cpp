...
bool isProjectNodeSupported(const core::ProjectNode* projectNode) {
  const auto* sourceNode =
      projectNode->sources().empty() ? nullptr : projectNode->sources()[0].get();

  // Reject projects with no outputs.
  if (projectNode->outputType()->size() == 0) {
    return false;
  }

  // Allow Projects on constant-only Values sources (output type size is 0) but
  // keep rejecting other empty sources.
  const bool sourceIsEmpty =
      (sourceNode == nullptr) || sourceNode->outputType()->size() == 0;
  if (sourceIsEmpty &&
      (sourceNode == nullptr ||
       dynamic_cast<const core::ValuesNode*>(sourceNode) == nullptr)) {
    return false;
  }
