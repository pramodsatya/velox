...
class CudfQueryConfig : public velox::config::ConfigBase {
 public:
  /// Default constructor.
  CudfQueryConfig();

  /// Constructor that initializes config from a map of values.
  explicit CudfQueryConfig(std::unordered_map<std::string, std::string>&& values);
  ...
  T get(const CudfQueryConfigEntry& entry) const {
    ...
    return velox::config::IConfig::get<T>(entry.name, defaultValue);
  }
...
class CudfSystemConfig : public velox::config::ConfigBase {
 public:
  ...
  /// Constructor that initializes config from a map of values.
  explicit CudfSystemConfig(std::unordered_map<std::string, std::string>&& values);
