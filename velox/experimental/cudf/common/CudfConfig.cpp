...
CudfQueryConfig::CudfQueryConfig(std::unordered_map<std::string, std::string>&& values)
...
CudfSystemConfig::CudfSystemConfig(std::unordered_map<std::string, std::string>&& values)
    : velox::config::ConfigBase(std::move(values), true) {
  validateConfigs();
}
