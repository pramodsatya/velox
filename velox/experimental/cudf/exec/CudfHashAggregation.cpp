...
std::unordered_map<std::string, exec::AggregateFunctionEntry>
getCudfAggregationFunctionSignatureMap() {
  std::unordered_map<std::string, exec::AggregateFunctionEntry> result;
  ...
  for (const auto& [name, stepMap] : registry) {
    ...
    exec::AggregateFunctionEntry entry;
    entry.signatures = std::move(aggregateSignatures);
    entry.metadata.orderSensitive = false;
